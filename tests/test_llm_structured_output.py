...
def test_action_rejects_unknown_intent() -> None:
    ...
    raw = json.dumps({
        "intent": "sing",
        "public_message": "", "target_name": None,
...
def test_response_schema_has_required_fields() -> None:
    ...
    assert set(schema["required"]) == {
        "intent", "public_message", "target_name", "reason_summary", "confidence",
    }
...
async def _seed_game(repo):
    game = Game(
        ...
        phase=Phase.NIGHT, day_number=1,
    ...
    seats = [
        Seat(seat_no=1, display_name="Human1", discord_user_id="u1", is_llm=False, persona_key=None),
...
async def test_llm_adapter_submits_night_action_for_seer(repo) -> None:
    ...
    decider = FakeLLMActionDecider(scripted=[
        LLMAction(intent="night_action", target_name="Human1", reason_summary="hunt wolves", confidence=0.9),
    ])
    adapter = LLMAdapter(
        repo=repo, decider=decider, rng=random.Random(0),
    )
...
async def test_llm_adapter_falls_back_on_invalid_target_name(repo) -> None:
    ...
    gs = FakeGameService()
    decider = FakeLLMActionDecider(scripted=[
        LLMAction(intent="night_action", target_name="NotAnyone", reason_summary="", confidence=0.5),
    ])
    adapter = LLMAdapter(
        repo=repo, decider=decider, rng=random.Random(0),
    )
...
async def test_llm_adapter_votes_with_name_resolution(repo) -> None:
    ...
    await adapter.submit_llm_votes(
        game, players, seats, candidates=None, round_=0
    )
