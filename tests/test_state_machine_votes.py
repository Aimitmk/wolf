...
def _v(voter: int, target: int | None, day: int = 1, round_: int = 0) -> Vote:
    return Vote(
        game_id="g1", day=day, round=round_, voter_seat=voter,
...
def test_unique_plurality_executes_and_advances_to_night() -> None:
    ...
    seats = _seats()
    votes = [
        _v(1, 7), _v(2, 7), _v(3, 7), _v(4, 7),
        _v(5, 8), _v(6, 8), _v(7, 9), _v(8, 9), _v(9, 1),
    ]
    t = plan_day_vote_resolve(
        game, players, seats, votes, force_skip=False, now_epoch=1000
    )
...
def test_tied_vote_goes_to_runoff() -> None:
    ...
    votes = [
        _v(1, 7), _v(2, 7),
        _v(3, 8), _v(4, 8),
        _v(5, 9), _v(6, 9), _v(7, 9),  # 9 gets 3 votes
...
def test_missing_vote_with_force_skip_treats_missing_as_abstain() -> None:
    ...
    t = plan_day_vote_resolve(
        game, players, seats, votes, force_skip=True, now_epoch=1000
    )
...
def test_runoff_tie_skips_execution() -> None:
    ...
    seats = _seats()
    votes = [
        _v(1, 7, round_=1), _v(2, 7, round_=1),
        _v(3, 8, round_=1), _v(4, 8, round_=1),
        _v(5, 7, round_=1), _v(6, 8, round_=1),
        _v(7, 7, round_=1), _v(8, 8, round_=1),
        _v(9, None, round_=1),
    ]
    t = plan_day_runoff_resolve(
        game, players, seats, votes,
        tied_candidates=[7, 8], force_skip=False, now_epoch=2000,
    )
...
def test_runoff_unique_executes() -> None:
    ...
    seats = _seats()
    votes = [
        _v(1, 7, round_=1), _v(2, 7, round_=1), _v(3, 7, round_=1),
        _v(4, 8, round_=1), _v(5, 8, round_=1),
        _v(6, 7, round_=1), _v(7, 8, round_=1),
        _v(8, 7, round_=1), _v(9, 8, round_=1),
    ]
    t = plan_day_runoff_resolve(
        game, players, seats, votes,
        tied_candidates=[7, 8], force_skip=False, now_epoch=2000,
...
def test_execution_triggering_victory_ends_game() -> None:
    ...
    t = plan_day_vote_resolve(
        game, players, seats, votes, force_skip=False, now_epoch=3000
    )
