...
Day flow: `DAY_DISCUSSION` (two discussion rounds for LLM seats) → `DAY_VOTE` → if a tie that includes LLM candidates: `DAY_RUNOFF_SPEECH` (one speech per tied LLM candidate) → `DAY_RUNOFF`. Whichever path produces the executed seat, if that seat `is_llm` the flow detours through `DAY_EXECUTION_SPEECH` (one last-words turn for the condemned LLM) before `_apply_execution` runs; human executions go straight to `_apply_execution` → `NIGHT` / `GAME_OVER`. Per-LLM-seat speech progress is tracked in the `llm_speech_counts` table (`discussion_rounds_done`, `runoff_speech_done`, `execution_speech_done`) so a force-skip / restart mid-flight resumes correctly without double-posting. `DAY_VOTE` and `DAY_RUNOFF` advances are additionally gated by `GameService._vote_resolution_due()` — votes resolve only when the deadline is reached, all votes are in, or the host force-skipped. A bare wake before the deadline is dropped, not parked in `WAITING_HOST_DECISION`.
...
- At game end, `domain/state_machine.py::_role_reveal_log` appends a single `ROLE_REVEAL`-kind `LogEntry` listing every seat's final role + alive/dead status. It is emitted from **both** win paths — the execution-victory branch in `_apply_execution` and the attack-victory branch in `plan_night_resolve`. Any new end-of-game transition must emit this log, or the public reveal will be missing.
